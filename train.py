    ...
    for epoch in range(
        ...
        if opt.grow_prune:
            ...
            if epoch >= 1 and epoch <= opt.T_max and epoch % opt.delta_T == 0:
                ...
                layer_ratio_up = regrow_allocation(
                    model, channel_ratio, layer_ratio_down
                )  # for grow
                ...
                cfg_mask, prev_model = update_mask(
                    model, layer_ratio_up, layer_ratio_down, prev_model, rank
                )
                apply_mask(model, cfg_mask)
                print(
                    "apply updated mask | detect channel sparsity: {}".format(
                        detect_channel_zero(model)
                    )
                )
